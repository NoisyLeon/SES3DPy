...
class noisetrace(obspy.core.trace.Trace):
    ...
    def aftan(self, pmf=True, piover4=-1.0, vmin=1.5, vmax=5.0, tmin=4.0, \
        ...
        #### For AGU !!!
        stla=tempsac.stats.sac.stla;
        stlo=tempsac.stats.sac.stlo;
        if stla < 27. and stlo < 110.:
            vmin=2.5;
        if stla>38. and stla < 46. and stlo < 95:
            vmin=2.5;
        if stla < 26. and stlo < 92.5:
            vmin=2.7;
        # azi=tempsac.stats.sac.az
        # if azi > 225 and azi < 300 and dist> 1400:
        #     vmin=2.5;
        # if azi > 225 and azi < 300 and dist> 2500:
        #     vmin=2.7;
        ####
    ...
    def getSNRRussell(self, foutPR='', pmf=True, piover4=-1.0, vmin=1.5, vmax=5.0, tmin=4.0, \
        tmax=30.0, tresh=20.0, ffact=1.0, taperl=1.0, snr=0.2, fmatch=1.0,phvelname=''):
        """getSNR
        Get the SNR for signal window based on FTAN analysis.
        If input noisetrace is double-lagged, it will do SNR analysis for pos/neg lag; otherwise it will do SNR analysis for sym lag.
        ...
        try:
            ...
            dt=self.stats.delta;
            dist=self.stats.sac.dist;
            Delta=obsGeo.kilometer2degrees(dist);
            if ( abs(begT+endT) < dt ):
                symFlag=True
                begT=0.
                temp_pos=self.getpos()
            ...
            if symFlag==True:
                ...
                ### positive lag
                for i in np.arange(fparam_p.nfout2_2):
                    fcorner=0.6/o_per_p[i]/np.sqrt(Delta);
                    fmin=1./o_per_p[i]-fcorner;
                    fmax=1./o_per_p[i]+fcorner;
                    filter_Trace=temp_pos.copy();
                    filter_Trace.filter('bandpass', freqmin=fmin, freqmax=fmax, corners=3, zerophase=True);
                    filtered_tr=filter_Trace.data;
                    # filtered_tr=temp_pos.GaussianFilter(1./o_per_p[i], fhlen=fhlen)
                    minT = dist/g_vel_p[i]-o_per_p[i]/2.
                    maxT = dist/g_vel_p[i]+o_per_p[i]/2.
                    if g_vel_p[i]<0 or o_per_p[i]<0:
                ...
                for i in np.arange(fparam_n.nfout2_2):
                    ...
                    fcorner=0.6/o_per_n[i]/np.sqrt(Delta);
                    fmin=1./o_per_n[i]-fcorner;
                    fmax=1./o_per_n[i]+fcorner;
                    filter_Trace=temp_neg.copy();
                    filter_Trace.filter('bandpass', freqmin=fmin, freqmax=fmax, corners=3, zerophase=True);
                    filtered_tr=filter_Trace.data;
                ...
                for i in np.arange(fparam.nfout2_2):
                    ...
                    fcorner=0.6/o_per[i]/np.sqrt(Delta);
                    fmin=1./o_per[i]-fcorner;
                    fmax=1./o_per[i]+fcorner;
                    filter_Trace=self.copy();
                    filter_Trace.filter('bandpass', freqmin=fmin, freqmax=fmax, corners=3, zerophase=True);
                    filtered_tr=filter_Trace.data;
...
class StaInfo(object):
    ...
    def GetAmpDist(self, SLst, per, datadir, outdir, dirtin, \
            mindist, maxdist, tin='COR', chpair=['LHZ', 'LHZ'] ):
        """
        Input:
        ...
        for station2 in SLst.stations:
            if station1.stacode>=station2.stacode and self.type=='station':
                continue
            dist, az, baz=obsGeo.gps2DistAzimuth(station1.lat, station1.lon, station2.lat, station2.lon )
            dist=dist/1000;
            Delta=obsGeo.kilometer2degrees(dist);
            ...
            if self.type=='station':
                ...
                sacfname=datadir+"/"+dirtin+"/"+sta1+"/"+station2.network+'.'+station2.stacode+'..'+chpair[0]+'.SAC';
            
            if (not os.path.isfile(sacfname+'_2_DISP.1') or not os.path.isfile(sacfname+'_amp_snr')) and self.type=='station':
            ...
            if signal1<0:
                continue;
            Ms=GetMs(signal1, per, dist);
            LonLst=np.append(LonLst, station2.lon)
            LatLst=np.append(LatLst, station2.lat)
            AmpLst=np.append(AmpLst, signal1)
            AziLst=np.append(AziLst, az)
            MagLst=np.append(MagLst, Ms)
            Length=Length+1
        Ampfout=np.append(LonLst, LatLst)
        Ampfout=np.append(Ampfout, AziLst)
        Ampfout=np.append(Ampfout, AmpLst)
        Ampfout=np.append(Ampfout, MagLst)
        Ampfout=Ampfout.reshape((5,Length))
        Ampfout=Ampfout.T
        Ampfoutname=outdir+'/Amp_Dist_'+station1.stacode+'.'+str(mindist)+'_'+str(maxdist)+'.txt';
        print Ampfoutname
        np.savetxt(Ampfoutname, Ampfout, fmt='%g')
        return 
    
    
    def GetAmpAzi(self, SLst, per, datadir, outdir, dirtin, \
            minazi, maxazi, tin='COR', chpair=['LHZ', 'LHZ'] ):
        """
        Input:
        SLst - station list( a StaLst object )
        per - period
        ...
        for station2 in SLst.stations:
            ...
            if signal1<0:
                continue;
            # C=1.66*math.log10(Delta)+0.3;
            Ms=GetMs(signal1, per, dist);
            LonLst=np.append(LonLst, station2.lon)
            LatLst=np.append(LatLst, station2.lat)
            AmpLst=np.append(AmpLst, signal1)
            DistLst=np.append(DistLst, dist)
            MagLst=np.append(MagLst, Ms)
            Length=Length+1
        Ampfout=np.append(LonLst, LatLst)
        Ampfout=np.append(Ampfout, DistLst)
        Ampfout=np.append(Ampfout, AmpLst)
        Ampfout=np.append(Ampfout, MagLst)
        Ampfout=Ampfout.reshape((5,Length))
        Ampfout=Ampfout.T
        Ampfoutname=outdir+'/Amp_Azi_'+station1.stacode+'.'+str(minazi)+'_'+str(maxazi)+'.txt';
        print Ampfoutname
        np.savetxt(Ampfoutname, Ampfout, fmt='%g')
        return 
    
    
    def GetAmplitudeFile(self, SLst, per, datadir, outdir, dirtin, \
            minlon, maxlon, minlat, maxlat, tin='COR', dx=0.2, chpair=['LHZ', 'LHZ'] ):
        """
        ...
        """
        station1=self
        Tfname=outdir+'/Amplitude_'+station1.stacode+'.c.txt'
        f=open(Tfname,'wb') 
        LonLst=np.array([])
        LatLst=np.array([])
        ALst=np.array([])
        ...
        for station2 in SLst.stations:
            ...
            dist=dist/1000.
            if math.isnan(signal1) or math.isnan(pvel) or math.isnan(snr):
                print 'NaN Detected for:',sacfname
                continue;
            f.writelines('%g %g %g %g %s %d \n' % ( station2.lon, station2.lat, signal1, pvel, station2.stacode, int(snr) ) )
           
            if snr > 0 or ( self.datatype=='ses3d' and signal1>0):
                LonLst=np.append(LonLst, station2.lon)
                LatLst=np.append(LatLst, station2.lat)
                ALst=np.append(ALst, signal1)
                Length=Length+1
        try:
            lsnr=snr
        except:
        ...
        # ALst=np.append(ALst, 0.)
        # Length=Length+1
        if Length==1:
            return
        tempTout=np.append(LonLst, LatLst)
        tempTout=np.append(tempTout, ALst)
        tempTout=tempTout.reshape((3,Length))
        tempTout=tempTout.T
        tempoutfname=outdir+'/'+sta1+'_Amplitude.lst'
        TfnameHD=Tfname+'.HD'
    ...
    def GetTravelTimeFile(self, SLst, per, datadir, outdir, dirtin, \
        ...
        for station2 in SLst.stations:
            ...
            if snr > 0 or ( self.datatype=='ses3d' and TravelT>0):
                LonLst=np.append(LonLst, station2.lon)
                LatLst=np.append(LatLst, station2.lat)
    ...
    def CheckTravelTimeCurvature(self, per, outdir, minlon, npts_x, minlat, npts_y, dx=0.2, filetype='phase'):
        ...
        for tl in f.readlines():
            ...
            tname = tl1[4];
            tflag = int(tl1[5]);
            if (tflag<=0 and self.datatype=='real') or (temp2<0):
            # # # if (temp2<=0):
        ...
        for i in np.arange(npts_x):
            for j in np.arange(npts_y):
                ...
                for ista in np.arange(len(lonin)):
                    lon2=lonin[ista];
                    lat2=latin[ista];
                    # Values that may need to modified!!!
                    if abs(lon2-lon) > cdist/110. or abs(lat2-lat) > cdist/110.:
                    # # # if abs(lon2-lon) > cdist/50. or abs(lat2-lat) > cdist/50.:
                        continue;
                    if lon2-lon<0:
                        marker_E=0;
                    else:
...
class StaLst(object):
    ...
    def GetAmplitudeFile(self, SLst, perlst, datadir, outdir, dirtin, \
            minlon, maxlon, minlat, maxlat, tin='COR', dx=0.2, chpair=['LHZ', 'LHZ'] ):
        for per in perlst:
            outdirin=outdir+'/'+str(int(per))+'sec';
            if not os.path.isdir(outdirin):
                os.makedirs(outdirin)
            for station in self.stations:
                station.GetAmplitudeFile(SLst=SLst, per=per, datadir=datadir, outdir=outdirin, dirtin=dirtin,\
                    minlon=minlon, maxlon=maxlon, minlat=minlat, maxlat=maxlat, tin=tin, dx=dx, chpair=chpair );
        print 'End of Getting Amplitude Maps for Helmhotz Tomography!'
        return;
    ...
    def GenerateReceiverLst(self, minlat, maxlat, minlon, maxlon, dlat, dlon, factor=100, net='LF', PRX='LF'):
        ...
        for lon in LonLst:
            for lat in LatLst:
                ...
                lat=lat;
                elevation=0.0;
                stacode=PRX+str(strlon)+'S'+str(strlat);
        ...
        for chan1 in Chan1:
            for chan2 in Chan2:
                if self.datatype=='noise':
                    ...
                    sta2=lon2+'S'+lat2;
                    ###
                    sacfname=datadir+"/"+sta1+"/"+self.station2.network+"."+sta2+'..'+chan2+".SAC";
                    foutPR=outdir+"/"+dirtout+"/"+sta1+"/"+self.station2.network+"."+sta2+'..'+chan2+".SAC";
                    Fname=self.station2.network+"."+sta2+'..'+chan2+".SAC";
                ...
                if self.datatype=='earthquake':
                    Odir=outdir+"/"+dirtout+"/"+sta1;
                    if not os.path.isdir(Odir):
                        os.makedirs(Odir);
                try:
                ...
                if inftan.dosnrflag==True:
                    ...
                    #     tmax=inftan.tmax, tresh=inftan.tresh, ffact=inftan.ffact, taperl=inftan.taperl, snr=inftan.snr, fmatch=inftan.fmatch, phvelname=self.phvelname)
                    tr.getSNRRussell(foutPR=foutPR, pmf=inftan.pmf, piover4=inftan.piover4, vmin=inftan.vmin, vmax=inftan.vmax, tmin=inftan.tmin, \
                        tmax=inftan.tmax, tresh=inftan.tresh, ffact=inftan.ffact, taperl=inftan.taperl, snr=inftan.snr, fmatch=inftan.fmatch, phvelname=self.phvelname)
                    if self.datatype=='noise':
                        tr.makesym()
                        # tr.getSNRRussell(fhlen=inftan.fhlen)
                        tr.getSNRRussell()
...
class StaPairLst(object):
    ...
    def GenerateEventStaPairLst(self, eventlst, staLst, stationflag=0, chanAll=[['BHZ']] ):
        """
        Input:
        eventlst: Event List  (a Event List)
        staLst: Station List  (a StaLst object)
        stationflag:
            0: sta1 < sta2; 1: sta1 <= sta2; 2: any
        chanAll: Channel List ( MUST be a nested list)
        ...
        for event in eventlst:
            for sta in staLst:
                for chan in chanAll:
                    tempPair=staPair(sta1=event, sta2=sta)
                    tempPair.station1.setChan([['']])
                    tempPair.station2.setChan(chan)
                    tempPair.datatype='earthquake'
                    self.append(tempPair)
        return
...
def GetMs(Amp, per, dist, method='Russell'):
    Delta=obsGeo.kilometer2degrees(dist);
    if method=='Russell':
        fcorner=0.6/per/np.sqrt(Delta);
        Ms=np.log10(Amp) + 0.5*np.log10( np.sin(Delta*math.pi/180.) ) + 0.0031*((20./per)**1.8)*Delta\
            - 0.66*np.log10(20./per)-np.log10(fcorner)-0.43;
    else:    
        C=1./3.*math.log10(Delta)+0.5*math.log10(math.sin(Delta*math.pi/180.))+0.0046*Delta+5.37;
        # C=1.094*math.log10(Delta)+4.429;
        Ms=math.log10(Amp/per/1000.)+C;
    return Ms;
