...
class h5fields(h5py.File):
    ...
    def convert_to_vts(self, outdir, component, iter0=0, iterf=30000, diter=100, Radius=0.98, verbose=True):
        """
...
class ses3d_fields(object):
    ...
    def conver_to_hdf5(self, outfname, component, depth,  iter0=0, iterf=30000, diter=100 ):
        """
        Plot depth slices of field component at given depth ranging between "valmin" and "valmax"
        ================================================================================================
        Input parameters:
    ...
    def conver_to_vts(self, component, depth,  iteration=14000, stations=True,
        ...
        for p in xrange(n_procs):
            ...
            if (radius >= self.z[p,:].min()) & (radius <= self.z[p,:].max()):
                idz=min(np.where(min(np.abs(self.z[p,:]-radius))==np.abs(self.z[p,:]-radius))[0])
                # - Read this field and make lats & lons. ------------------------------------------
                field   = np.append(field, self.read_single_box(component, p, iteration)[:,:,idz])
                theta1, phi1 = np.meshgrid(self.theta[p,:], self.phi[p,:], indexing='ij')
                theta   = np.append(theta, theta1)
                phi     = np.append(phi, phi1)
                # r       = np.append(r, r1)
        return field
        # theta, phi, r = np.meshgrid(theta, phi, r, indexing='ij')
        x = r * np.sin(theta) * np.cos(phi)/Rref
        y = r * np.sin(theta) * np.sin(phi)/Rref
        z = r * np.cos(theta)/Rref
