...
class ATMFDataSet(h5py.File):
    ...
    def verticalExtend(self, inname, outname, block, refname ='ak135', dz=[], depthlst=[], z0lst =[], depthArr=np.array([]), maxdepth = 410.,
        ...
        dz, depthlst   - list for depth interval and bottom depth
        maxdepth      - maximum depth to extend
        ...
        if depthArr.size == 0:
            ...
            depthArr=np.append(depthArr, depthlst[-1]+z0lst[-1])
        # create group name
        ...
        if (maxdepth > depthArr[-1] and (maxdepth - depthArr[-1]) % dz2 ==0.):
            step2 = True
            if maxdepth > 410.:
        ...
        for index in self[inname].keys():
            ...
            if step2 ==True:
                ...
                Qak135_410=146.57
                if maxdepth > 410.:
                    depInter2=depthInter[-1]+np.arange((410. - depthInter[-1])/dz2+1)*dz2 # depth array for interpolation
                else:
                    depInter2=depthInter[-1]+np.arange((maxdepth - depthInter[-1])/dz2+1)*dz2 # depth array for interpolation
    ...
    def horizontalExtend(self, modelname, minlat, maxlat, dlat, minlon, maxlon, dlon,
        ...
        for lon in lonarr:
            for lat in latarr:
                ...
                for index in self[modelname].keys():
                    elat = self[modelname][index].attrs['lat']
                    elon = self[modelname][index].attrs['lon']
    ...
    def horizontalExtendMP(self, modelname, minlat, maxlat, dlat, minlon, maxlon, dlon,
        ...
        HEXTRAPOLATE = partial(PositionHExtrapolate, avgArr=avgArr,
                    outdir=outdir, dataset=self[modelname], Dref=Dref)
        pool =mp.Pool()
        ...
        print 'End of Adding Horizontal Slowness  ( MP ) !'
        return 
