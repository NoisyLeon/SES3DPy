...
def PlotVectorMap(fname, maxlat, minlat, maxlon, minlon, \
    dlon=0.2, dlat=0.2, title='', datatype='ph', outfname='', res='i',\
    browseflag=True, saveflag=False, mapflag='regional_ortho', mapfactor=2.5):
    """
    Plot Tomography Map
    ...
    angle=angle[(LatLst<maxlat-0.5)*(LatLst>minlat)]
    x,y = m(lon, lat)
    ...
    # m.pcolor(xi, yi, zi, cmap='jet', shading='gouraud');
    
    U=np.cos(angle*np.pi/180.);
    V=np.cos(angle*np.pi/180.);
    uout, vout=m.transform_vector(U,V,lon[0],lat[:,0],100,100,returnxy=True,masked=True)
    m.quiver(x, y, uout, vout);
    # m.quiver(x[:20], y[:20], U[:20], V[:20], width=0.001);
    # print lat.max()
...
minlon=90.
maxlon=143.
PlotVectorMap(fname, maxlon=maxlon, minlon=minlon, maxlat=maxlat, minlat=minlat, dlon=dlon, dlat=dlat)
# fname='slow_phase.map'
# PlotVectorMap(fname, maxlon=maxlon, minlon=minlon, maxlat=maxlat, minlat=minlat)
#!/usr/bin/env python
