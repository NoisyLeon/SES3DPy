...
def PlotFile(fname, ax=plt.subplot(), FMT='', x=1, y1=2, y2=None, err1=None, err2=None, lw=3, markersize=5):
    ...
    X=Inarray[:,x-1]
    Y1=Inarray[:,y1-1]
    Delta=obspy.geodetics.kilometer2degrees(X)
    Y1=Y1*np.sin(Delta/180.*np.pi)/np.sin(Delta.min()/180.*np.pi)/Y1.max()
    # ax=plt.subplot()
    if err1==None:
        if FMT!='':
...
line1=PlotFile(fname, ax=ax, FMT='bo', x=1, y1=4, y2=None, err1=None, err2=None, lw=10, markersize=markersize)
minazi=253.
maxazi=254.
fname=datadir+'/az_'+str(minazi)+'_'+str(maxazi)+'_amp_Ms.lst'
line2=PlotFile(fname, ax=ax, FMT='go', x=1, y1=4, y2=None, err1=None, err2=None, lw=10, markersize=markersize)
minazi=300.
maxazi=301.
fname=datadir+'/az_'+str(minazi)+'_'+str(maxazi)+'_amp_Ms.lst'
line3=PlotFile(fname, ax=ax, FMT='ro', x=1, y1=4, y2=None, err1=None, err2=None, lw=10, markersize=markersize)
