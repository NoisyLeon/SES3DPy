...
class StaInfo(object):
    ...
    def SES3D2SAC(self, datadir, outdir, scaling=1e9, VminPadding=-1, integrate=True, delta=0.1):
        ses3dST=ses3d_seismogram();
        staname=self.network+'.'+self.stacode;
        if ses3dST.read(datadir, staname, integrate=integrate):
            obsST=ses3dST.Convert2ObsStream(self.stacode, self.network, VminPadding=VminPadding, scaling=scaling);
            for tr in obsST:
                Nd=int(delta/tr.stats.delta);
                if Nd>1:
                    tr.decimate(Nd, no_filter=True);
...
class StaLst(object):
    ...
    def SES3D2SACParallel(self, datadir, outdir, scaling=1e9, VminPadding=-1, integrate=True, delta=0.1):
        if not os.path.isdir(outdir):
            os.makedirs(outdir);
        Ses3D2SAC = partial(StationSES3D2SAC, datadir=datadir, outdir=outdir, \
                scaling=scaling, VminPadding=VminPadding, integrate=integrate, delta=delta);
        pool =mp.Pool()
    ...
    def WriteRecfile(self, outdir):
        L=len(self.stations);
        outfname=outdir+'/recfile_1';
        f=open(outfname,'wb');
        f.writelines('%g\n' %L )
        for station in self.stations:
...
class SES3DInputGenerator(InputFileGenerator):
    ...
    def StaLst2Generator(self, SList, minlon=-999, maxlon=999, minlat=-999, maxlat=999):
        L=0
        for station in SList:
            ...
            if lon<minlon or lon > maxlon or lat< minlat or lat > maxlat:
                continue;
            print 'Adding Station: ', station.stacode
...
class SourceTimeFunc(obspy.core.trace.Trace):
