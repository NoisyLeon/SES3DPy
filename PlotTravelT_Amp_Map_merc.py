...
def PlotContourMap(fname, maxlat, minlat, maxlon, minlon, \
    ...
    m.pcolormesh(xi, yi, zi, cmap='bwr_r', vmin=-0.05, vmax=0.05, shading='gouraud')
    ...
    # cb.set_label('Laplacian of T ', fontsize=20)
  ...
  #  m.plot(x, y, 'b-', lw=3)
    ...
    plt.suptitle('Laplacian of Travel Time', fontsize=22);
    if browseflag==True:
        plt.draw()
        plt.pause(1) # <-------
...
minlon=90.;
maxlon=143.;
#PlotContourMap(fname, browseflag=False, saveflag=True, mapflag='regional_merc', maxlon=maxlon, minlon=minlon, maxlat=maxlat, minlat=minlat, dlon=dlon, dlat=dlat)
